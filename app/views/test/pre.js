...
function checkScore() {
	...
	for (i = 0; i < scores.idPercentage.length; i++) {
		if (scores.idPercentage[i] === "--") {
			scoreIndex = i;
			break;
		} else if ((round(score / total) * 100) < scores.idPercentage[i]) {
			scoreIndex = i;
			break;
		}
	} 
...
function openEndModal() {
	animateModal("open", "end").then(function() {
		postIdData();
		removeEntries();
		openModal = "open";
	});
	...
	appSettings.setBoolean("preCompleted", true);
	scores.preScore = Math.round((score / total) * 100);
}
	...
	for (i = 0; i < dataEntries.entriesId.length; i++) {
		...
		if (entry.gameIndex === gameIndex) {
			...
			if (!entry.sent && entry.completed) {
				http.request({
					url: `https://rala.pals.ucl.ac.uk/Experiments/php/MA_ID.php?a=${entry.participant}&b=${entry.condition}&c=${entry.round}&d=${entry.F1talker}&e=${entry.F1vowel}&f=${entry.response}&g=${entry.correct}&q=125&z=1`,
					method: "POST"
				}).then((response) => {
					if (response.content === "\nConnectSuccess") {
						entry.sent = true;   
