...
function createDeck(cards) {
	...
	listChunk = [Math.floor(Math.random() * 2), Math.floor(Math.random() * 2)+2];
	...
	for (let i = 0; i < tempWord.length; i++) {
		...
		tmp = [tempWord[i], tempAudio[i], idMatch, symbolIndices[i]];
...
function removeIncompleteEntries() {
	for (i = 0; i < dataEntries.entriesCards.length; i++) {
		if (dataEntries.entriesCards[i].completed === false) {
...
function removeSentEntries() {
	for (i = 0; i < dataEntries.entriesCards.length; i++) {
		if (dataEntries.entriesCards[i].sent) {
			dataEntries.entriesCards.splice(i, 1);
		}
	}

	for (i = 0; i < dataEntries.entriesCardSummary.length; i++) {
		if (dataEntries.entriesCardSummary[i].sent) {
