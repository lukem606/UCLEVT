...
exports.onLoaded = function(args) {
	...
	talker = appSettings.getString("talker");
	showScores();
}
...
function animateDialog(location) {
	...
	modal.animate({opacity: 0, duration: 500})
		.then(function() {modal.style.visibility = "hidden"; });

	modal = page.getViewById("modal-dialog");
	modal.style.visibility = "visible";
	modal.animation({opacity: 1, duration: 500});
}
...
function openEndModal() {
	...
	appSettings.setNumber("gamesCount", appSettings.getNumber("gamesCount") + 1);
}
	...
	for (i = 0; i < dataEntries.entriesId.length; i++) {
		...
		if (entry.gameIndex === gameIndex) {
			...
			if (!entry.sent && entry.completed) {
				http.request({
					url: `https://rala.pals.ucl.ac.uk/Experiments/php/MA_ID.php?a=${entry.participant}&b=${entry.condition}&c=${entry.moves}&d=${entry.F1talker}&e=${entry.F1vowel}&f=${entry.response}&g=${entry.correct}&q=125`,
					method: "POST"
				}).then((response) => {
					if (response.content === "\nConnectSuccess") {
						entry.sent = true;   
...
function setupRound() {
	...
	entry.F1talker = speakerOrder[current] + 1;
	...
	if (talker === "Same RP Talker" || talker === "Different RP Talkers") {
		for (i = 0; i < set.length; i++) {
			speakStr = speakerRP[speakerOrder[current]];
			townStr = accentRP[speakerOrder[current]];
			audioList.push(routes.audio+"/"+townStr+"/"+speakStr+"/"
								+townStr+"_"+speakStr+"_"+set[i]+".mp3");
		}
	} else if (talker === "Different Accents") {
		for (i = 0; i < set.length; i++) {
			speakStr = speakerOther[speakerOrder[current]];
			townStr = accentOther[speakerOrder[current]];
			audioList.push(routes.audio+"/"+townStr+"/"+speakStr+"/"
								+townStr+"_"+speakStr+"_"+set[i]+".mp3");
		}
	}
...
function startGame(loc) {
	options.current = 1;
	animateModal("closed", loc)
		.then(function() {
		score = 0;
		previousWords = [];

		if (talker === "Fixed RP Talker") {
			for (i=0; i < total; i++) {
				speakerOrder.push(appSettings.getNumber("fixedRP"));
			}
		} else if (talker === "Same RP Talker") {
			let index = Math.round(Math.random() * (speakerRP.length - 1));
			for (i=0; i < total; i++) {
				speakerOrder.push(index)
			}
			...
			for (i=0; i < total; i++) {
				speakerOrder.push(index);
				index++;
				if (index === speakerOther.length) {
					index = 0;
				}
			}
		}

		speakerOrder = shuffle(speakerOrder);
