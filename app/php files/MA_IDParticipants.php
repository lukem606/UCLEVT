...
if ($key=="125") {
	$query = "SELECT DISTINCT subject FROM id ORDER BY subject";
	$data = mysqli_query($conn, $query);
        $rowN = $data -> num_rows;
        for ($i=0;$i<$rowN;$i++) {
                $row = mysqli_fetch_assoc($data);
                $subj =  $row['subject'];
            		$query2 = "SELECT moveN FROM id WHERE subject = '$subj'";
            		$data2 = mysqli_query($conn, $query2);
            		$blockN = mysqli_num_rows($data2);
                $query2 = "SELECT SUM(corr) AS val FROM id WHERE subject = '$subj'";
                $data2 = mysqli_query($conn, $query2);
                $row2 = mysqli_fetch_assoc($data2);
                $corrSum = $row2['val'];
                $corrAve = round($corrSum/$blockN,2);
            		$query2 = "SELECT MAX(date) AS val FROM id WHERE subject = '$subj'";
            		$data2 = mysqli_query($conn, $query2);
            		$row2 = mysqli_fetch_assoc($data2);
            		$dateE = $row2['val'];
                $query2 = "SELECT MIN(date) AS val FROM id WHERE subject = '$subj'";
      	        $data2 = mysqli_query($conn, $query2);
              	$row2 = mysqli_fetch_assoc($data2);
              	$dateS = $row2['val'];
